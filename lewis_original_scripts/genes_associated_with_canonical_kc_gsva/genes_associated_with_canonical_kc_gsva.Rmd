...
main_file <- 'genes_associated_with_canonical_kc_gsva/results/vobj_fit_contrasts.rda'

```

## This script identifies genes associated custom GSVA KC scores
...
dge$samples$gsva_canonical_kc <- db_gsva_custom_correct[rownames(dge$samples), 'gsva_canonical_kc']


dge <- calcNormFactors(dge)

design <- model.matrix(~ gsva_canonical_kc + sex + age_centered_impute, 
                       data = dge$samples)
...
# Define formula
form <- ~ gsva_canonical_kc + sex + age_centered_impute + (1 | patient_id/final_tissue_type_category)
...
write.csv(fgsea_res_HK %>% 
             select(-leadingEdge), 
           file="genes_associated_with_canonical_kc_gsva/results/fgsea_hallmark_results.csv", 
          row.names = FALSE)
...
write.csv(fgsea_res_REACTOME %>% 
            select(-leadingEdge), 
          file="genes_associated_with_canonical_kc_gsva/results/fgsea_reactome_results.csv", 
          row.names = FALSE)
...
PHENO %>% 
  arrange(final_tissue_type_category, gsva_canonical_kc, patient_id) -> PHENO
...
pheatmap(vobj$E[AllDEGs, rownames(PHENO)],
         ...
         annotation_col=PHENO %>% select(final_tissue_type_category, gsva_canonical_kc),
         ...
         #cellwidth = 5,
         #cellheight = 10,
         main='All genes associated with custom GSVA KC KC proportion',
...
PHENO %>% 
  arrange(gsva_canonical_kc, patient_id) -> PHENO
...
pheatmap(vobj$E[AllDEGs, rownames(PHENO)],
         ...
         #cellwidth = 5,
         #cellheight = 10,
         main='All genes associated with custom GSVA KC score',
         #gaps_col = which(!duplicated(PHENO$final_tissue_type_category))[-1]-1,
         scale = 'row')
...
load(file='genes_associated_with_canonical_kc_gsva/results/vobj_fit_contrasts.rda')
