...
'data/raw/Danaher_Signature.csv'

creates data:

'data/processed/db_ssgsea_custom_correct.rda' 
'data/processed/db_gsva_custom_correct.rda'
...
+ Calculates and saves enrichment scores (ssGSEA & GSVA) for additional signatures of interest
  * Such as NMD reactome, Danaher Immune Infiltration, Cannonical Keratinocyte
...
reactome_list$REACTOME_INTERFERON_GAMMA_SIGNALING -> REACTOME_INTERFERON_GAMMA_SIGNALING

save(REACTOME_INTERFERON_GAMMA_SIGNALING,
     file='REACTOME_INTERFERON_GAMMA_SIGNALING.rda')

geneset_list$NMD_Reactome = name_vec[reactome_list$REACTOME_NONSENSE_MEDIATED_DECAY_NMD]
names(geneset_list$NMD_Reactome) <- NULL

str_subset(names(reactome_list), 'GAMMA')

geneset_list$IFG_Reactome = name_vec[reactome_list$REACTOME_INTERFERON_GAMMA_SIGNALING]
names(geneset_list$IFG_Reactome) <- NULL
geneset_list$IFG_Reactome <- geneset_list$IFG_Reactome[complete.cases(geneset_list$IFG_Reactome)]

geneset_list$MAPK_onco_reactome = name_vec[reactome_list$REACTOME_ONCOGENIC_MAPK_SIGNALING]
names(geneset_list$MAPK_onco_reactome) <- NULL
geneset_list$MAPK_onco_reactome <- geneset_list$MAPK_onco_reactome[complete.cases(geneset_list$MAPK_onco_reactome)]

geneset_list$MAPK_reactome = name_vec[reactome_list$REACTOME_MAPK_FAMILY_SIGNALING_CASCADES]
names(geneset_list$MAPK_reactome) <- NULL
geneset_list$MAPK_reactome <- geneset_list$MAPK_reactome[complete.cases(geneset_list$MAPK_reactome)]

read_csv('data/raw/Danaher_Signature.csv') %>%
  filter(Selected == 1) -> immune_infiltration

geneset_list$immune_infiltration = name_vec[immune_infiltration$Gene]
names(geneset_list$immune_infiltration) <- NULL

geneset_list$immune_infiltration <- geneset_list$immune_infiltration[complete.cases(geneset_list$immune_infiltration)]

keratinocyte_gene_list <- c('KRT1', 'KRT2', 'KRT5', 'KRT10', 'KRT14', 'IVL', 
                            'FLG', 'LOR', 'TGM3', 
                            'SPRR2A', 'DSG1', 'DSC1', 'PI3', 'DMKN')

geneset_list$canonical_kc = name_vec[keratinocyte_gene_list]
names(geneset_list$canonical_kc) <- NULL
geneset_list$canonical_kc <- geneset_list$canonical_kc[complete.cases(geneset_list$canonical_kc)]
...
colnames(ssgsea_score_results) <- paste0('ssgsea_', colnames(ssgsea_score_results))

cbind(meta_data, ssgsea_score_results[rownames(meta_data),]) -> db_ssgsea_custom_correct

db_ssgsea_custom_correct

save(db_ssgsea_custom_correct, 
     file='data/processed/db_ssgsea_custom_correct.rda')
...
colnames(gsva_score_results) <- paste0('gsva_', colnames(gsva_score_results))
  
cbind(meta_data, gsva_score_results[rownames(meta_data),]) -> db_gsva_custom_correct

save(db_gsva_custom_correct, 
     file='data/processed/db_gsva_custom_correct.rda')
...
db_ssgsea_custom_correct %>%
  ...
  pivot_longer(cols=contains('ssgsea'),
               names_to = 'signature',
               values_to = 'ssGSEA') %>%
  mutate(signature = str_remove(signature, 'ssgsea_')) -> db_stats
...
lapply(unique(db_stats$signature), function(L){
  
  ##print(L)
  
  db_stats %>%
    filter(signature == L) -> temp
...
results_db %>% 
  ...
  arrange(signature, y.position) %>%
  group_by(signature, y.position) %>%
...
save(results_db_ssgsea, db_stats_ssgsea,
     file='ssgsea_gsva_on_additional_signatures/results/results_db_ssgsea.rda')
...
db_gsva_custom_correct %>%
    filter(complete.cases(final_tissue_type_category)) %>%
  pivot_longer(cols=contains('gsva'),
               names_to = 'signature',
               values_to = 'GSVA') %>%
  mutate(signature = str_remove(signature, 'gsva_')) -> db_stats
...
save(results_db_gsva, db_stats_gsva,
     file='ssgsea_gsva_on_additional_signatures/results/results_db_gsva.rda')
...
Let's take a look at Danaher immune_infiltration signature.
...
```{r eval=T,  fig.width=6.5, fig.height=4}

## put name of gene-set of interest here

gs_of_interest <- 'immune_infiltration'

load(file='ssgsea_gsva_on_additional_signatures/results/results_db_gsva.rda')
load(file='ssgsea_gsva_on_additional_signatures/results/results_db_ssgsea.rda')

results_db_gsva %>% 
  ungroup() %>%
  filter(p.value.signif != 'ns',
         signature == gs_of_interest) %>% 
  arrange(signature, y.position) %>%
  group_by(signature, y.position) %>%
  #filter(signature=='Eosinophils') %>%
  mutate(group_numbered = row_number(),
         ymod=(group_numbered-1) * DIFF,
         y.position=y.position+ymod) %>%
  ungroup() -> results_db_filter
...
db_stats_gsva %>%
    filter(signature == gs_of_interest) %>% 
  ggplot(aes(x = final_tissue_type_category, 
             y = GSVA,
             color = final_tissue_type_category)) +
  geom_boxplot(outliers = F)+
  ...
  stat_pvalue_manual(data=results_db_filter, 
                     ...
                     tip.length = 0)+
  scale_y_continuous(expand = expansion(mult=0.1))+
  facet_wrap(~signature, scales='free', ncol=5)+
  theme_bw()+
  theme(legend.position = 'bottom')
...
#### Heatmaps visualizing Danaher immune infiltration genes
...
unique(name_vec[immune_infiltration$Gene]) -> gs_genes

gs_genes[!is.na(gs_genes)] -> gs_genes

dge_final$samples -> pheno

pheno$gsva_scores <- db_gsva_custom_correct[[paste0('gsva_', gs_of_interest)]]
...
log_cpm[gs_genes, rownames(pheno)] %>%
  apply(MARGIN=1, FUN=mean) %>% 
  sort(decreasing = T) -> gs_genes_ord

pheatmap(log_cpm[names(gs_genes_ord), rownames(pheno)],
         breaks = BREAKS,
...
pheno$ssgsea_scores <- db_ssgsea_custom_correct[[paste0('ssgsea_', gs_of_interest)]]
...
pheno %>% 
  arrange(final_tissue_type_category, gsva_scores) -> pheno
...
pheno %>% 
  arrange(final_tissue_type_category, ssgsea_scores) -> pheno
