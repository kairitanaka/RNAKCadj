...
main_file <- 'tissue_type_degs_xcell_kc_adjust/results/vobj_fit_contrasts.rda'
...
# Define formula
form <- ~ final_tissue_type_category + sex + age_centered_impute + Keratinocytes + (1 | patient_id)
...
save(fit, vobj, dge,
     file='tissue_type_degs_xcell_kc_adjust/results/vobj_fit_contrasts.rda')
...
# choose your cores
bp <- SnowParam(workers = 16, type = "SOCK", progressbar = TRUE)
register(bp)
...
write.csv(fgsea_res_HK %>% 
             select(-leadingEdge), 
           file="tissue_type_degs_xcell_kc_adjust/results/fgsea_hallmark_results.csv", row.names = FALSE)
...
write.csv(fgsea_res_REACTOME %>% 
            select(-leadingEdge), 
          file="tissue_type_degs_xcell_kc_adjust/results/fgsea_reactome_results.csv", row.names = FALSE)
...
## Volcano plots, xCell KC adjustment

model1 = final_tissue_type_category + sex + age_centered_impute + Keratinocytes + (1|patient_id)
...
## Heatmap, xCell KC adjustment

FCH > 2, FDR < 0.05

model = final_tissue_type_category + sex + age_centered_impute + Keratinocytes + (1 | patient_id)
...
pheatmap(vobj$E[AllDEGs, rownames(PHENO)],
         ...
         annotation_col=PHENO %>% select(final_tissue_type_category, 
                                         prog_type),
         cluster_cols = F,
         main='DEGs FCH > 2, FDR < 0.05, xCell KC adjustment',
...
form2: adjust for sex, age_centered_impute, patient_id, Keratinocytes and mean-variance trend
...
form_list <- list(form1 = ~ sex + age_centered_impute + (1|patient_id),
                  ...
                  form2 = ~ sex + age_centered_impute + Keratinocytes + (1|patient_id))
...
save(fit_reduced_list, form_list, 
     file='tissue_type_degs_xcell_kc_adjust/results/pca_stuff/fit_reduced_list_for_pca.rda')
...
dge_final$samples <- bind_cols(dge_final$samples, cell_data[rownames(dge_final$samples), 'Keratinocytes', drop=F])
...
lapply(names(form_list) , function(L){
  ...
  pca_result$x %>% 
    bind_cols(dge_final$samples) %>%
    mutate(Keratinocytes = as.numeric(scale(Keratinocytes, center = TRUE, scale = TRUE))) %>%
    ggplot(aes(x=PC1, y=PC2,
               color=Keratinocytes))+
